import { AddSvg, ArchiveSvg, BackArrowSvg, RestoreSvg, SettingsSvg, StatusSvg, TagsSvg, TimeSvg, TrashSvg } from "../Svg";
...
export default function TagDetails() {
  ...
  return <>{screenSize >= 1440 ? <TagDetailsDesktop noteData={noteData} setNoteData={setNoteData} /> : <TagDetailsMobile noteData={noteData} />}</>;
}

function TagDetailsMobile({ noteData }) {
...
function TagDetailsDesktop({ noteData, setNoteData }) {
  ...
  const currentTag = decodeURI(location.hash.substring(1).split("/").at(-1));

  useEffect(() => {
    setFilterData(noteData.filter((note) => note.tags.includes(currentTag)).filter((note) => note.title.toLowerCase().includes(filterText.trim().toLowerCase()) || note.note.toLowerCase().includes(filterText.trim().toLowerCase()) || note.tags.map((tag) => tag.toLowerCase().includes(filterText.trim().toLowerCase())).includes(true)));
  }, [filterText, noteData]);

  useEffect(() => {
    setSelectedNote(noteData.find((note) => note.id === location.hash.substring(1).split("/").at(-1)) || null);
    setFilterText("");
    setFilterData(noteData.filter((note) => note.tags.includes(currentTag)));
  }, [router]);
  ...
  return (
    <main>
      <DesktopHeader title="Tag Details" filterText={filterText} setFilterText={setFilterText} tag={currentTag} />
      <div className="page-container-desktop">
        <div className="page-left">
          <button onClick={() => setSelectedNote(null)}>+ Create New Note</button>
          <p>All notes with the ”{currentTag}” tag are shown here.</p>
          <ul className="notes-list">
            {filterData.map((note) => (
